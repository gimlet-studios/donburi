...
// MarshalBinary marshals the TransformData into a binary message, totalling ~20 bytes.
// this is a lossy conversion because every float64 gets casted down to a 32 bit float.
func (t *TransformData) MarshalBinary() ([]byte, error) {
	px := math.Float32bits(float32(t.LocalPosition.X))
	py := math.Float32bits(float32(t.LocalPosition.Y))

	pos := uint64(px) | (uint64(py) << 32)
	rot := float32(t.LocalRotation)

	sx := math.Float32bits(float32(t.LocalScale.X))
	sy := math.Float32bits(float32(t.LocalScale.Y))

	scale := uint64(sx) | (uint64(sy) << 32)
...
// UnmarshalBinary unmarshals the provided binary message into a TransformData object,
// this data is assumed to be generated by TransformData.MarshalBinary
func (t *TransformData) UnmarshalBinary(data []byte) error {
	var (
	...
	binary.Read(buf, binary.LittleEndian, &rot)
	...
	sy, sx := uint32(scale&0xFFFFFFFF), uint32((scale>>32)&0xFFFFFFFF)
	t.LocalScale.X, t.LocalScale.Y = float64(math.Float32frombits(sx)), float64(math.Float32frombits(sy))
